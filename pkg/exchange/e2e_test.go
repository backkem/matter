...
type TestManagerPair struct {
	...
	transport      *transport.PipeManagerPair
	received       [2]chan ReceivedTestMessage
	handlerWrapper [2]*exchangeHandlerWrapper
}

// ReceivedTestMessage represents a message received by a test manager.
type ReceivedTestMessage struct {
	Opcode      uint8
	Payload     []byte
	ExchangeID  uint16
	Unsolicited bool
...
type TestManagerPairConfig struct {
	// UDP enables UDP transport (default: true)
	UDP bool
	// TCP enables TCP transport (default: false)
	TCP bool
}

// exchangeHandlerWrapper routes transport messages to exchange manager.
// Needed to solve chicken-and-egg: transport needs handler, handler needs exchange manager.
...
// Use this for E2E tests that exercise the full exchange layer.
func NewTestManagerPair(config TestManagerPairConfig) (*TestManagerPair, error) {
	...
	pair := &TestManagerPair{
		received: [2]chan ReceivedTestMessage{
			make(chan ReceivedTestMessage, 100),
			make(chan ReceivedTestMessage, 100),
		},
	}

	// Create handler wrappers (will be populated with managers later)
	pair.handlerWrapper[0] = &exchangeHandlerWrapper{}
	pair.handlerWrapper[1] = &exchangeHandlerWrapper{}

	// Create transport pair with wrapper handlers
	...
	pair.transport = transportPair

	// Create protocol handlers that forward to channels
	for i := 0; i < 2; i++ {
		idx := i // capture for closure
		pair.handlers[i] = &testProtocolHandler{}
		pair.handlers[i].onReceive = func(msg ReceivedTestMessage) {
			select {
	...
	// Create session managers (needed for OnMessageReceived to look up sessions)
	pair.sessionMgrs[0] = session.NewManager(session.ManagerConfig{})
	pair.sessionMgrs[1] = session.NewManager(session.ManagerConfig{})

	// Create sessions for sending messages.
	// Use unique ephemeral node IDs: 0x1000 for side 0, 0x2000 for side 1
	pair.sessions[0] = newTestUnsecuredSession(0x1000)
	pair.sessions[1] = newTestUnsecuredSession(0x2000)

	// Create exchange managers and wire up everything
	for i := 0; i < 2; i++ {
		...
		// Wire handler wrapper to exchange manager
		pair.handlerWrapper[i].manager = pair.managers[i]

		// Register the test protocol handler
...
// Use this when creating exchanges: pair.Manager(0).NewExchange(pair.Session(0), ...)
...
// Use PeerAddress(1) when sending FROM manager 0 TO manager 1.
func (p *TestManagerPair) PeerAddress(idx int, tcp bool) transport.PeerAddress {
	addrs := p.transport.PeerAddresses(idx)
	if tcp {
		return addrs.TCP
	}
	return addrs.UDP
...
func (p *TestManagerPair) WaitForMessage(idx int, timeout time.Duration) (ReceivedTestMessage, bool) {
	select {
	case msg := <-p.received[idx]:
		return msg, true
	case <-time.After(timeout):
		return ReceivedTestMessage{}, false
	}
}

// Pipe returns the underlying pipe for network simulation.
func (p *TestManagerPair) Pipe() *transport.Pipe {
	return p.transport.Pipe()
}
...
// Used by TestManagerPair for exchange-to-exchange E2E testing.
// The receiver's exchange.Manager will find/create an UnsecuredContext for the source.
...
func (s *testUnsecuredSession) Encrypt(header *message.MessageHeader, protocol *message.ProtocolHeader, payload []byte, privacy bool) ([]byte, error) {
	// Create unsecured message with session ID 0 and source node ID
	s.mu.Lock()
	s.counter++
	header.MessageCounter = s.counter
	s.mu.Unlock()

	// Session ID 0 indicates unsecured session
	header.SessionID = 0
	// Source must be present for unsecured messages (per Spec 4.13.2.1)
...
type testProtocolHandler struct {
	...
	onReceive func(ReceivedTestMessage)
	mu        sync.Mutex
}
...
func (h *testProtocolHandler) OnMessage(ctx *ExchangeContext, opcode uint8, payload []byte) ([]byte, error) {
	...
	if h.onReceive != nil {
		h.onReceive(ReceivedTestMessage{
			Opcode:     opcode,
			Payload:    msg.Payload,
			ExchangeID: ctx.ID,
		})
...
func (h *testProtocolHandler) MessageCount() int {
	h.mu.Lock()
	defer h.mu.Unlock()
